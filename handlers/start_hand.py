...
from middleware.report_ware import Reportware, Myware
...
from utils.decorators import test_deco, for_all_methods
...
class Start_router():
    ...
    async def adv_company(message: Message, bot: Bot, state: FSMContext, command: CommandObject):
        ...
        await Start_router.commands_start(message, bot, state)


    @router.message(commands=['testdec'], state='*', flags=flags)
    @test_deco
    async def adv_company(message: Message, bot: Bot, state: FSMContext, command: CommandObject):
        await message.answer("decoratortest")
    ...
    async def commands_start(update: Message | CallbackQuery, bot: Bot, state: FSMContext):  # Первое сообщение
        ...
        asyncio.create_task(Start_router.start_base(user_obj))
        await state.clear()
