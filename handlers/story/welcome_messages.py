...
async def message_3(message: types.Message, state: FSMContext):  # Начало опроса
    ...
    all_count = await mongo_count_docs('database', 'statistics_new', [{'war_or_not': {'$exists': True}}, {'datetime': {'$gte': release_date['2_1']}}], hard_link=True)
    war = await mongo_count_docs('database', 'statistics_new', [{'war_or_not': '2️⃣ Война'}, {'datetime': {'$gte': release_date['2_1']}}], hard_link=True)
    not_war = await mongo_count_docs('database', 'statistics_new',
                                     [{'war_or_not': '1️⃣ Специальная военная операция (СВО)'}, {'datetime': {'$gte': release_date['2_1']}}], hard_link=True)
    FSB_not_war = await mongo_count_docs('database', 'statistics_new',
                                         [{'FSB': "Да"}, {'war_or_not': '1️⃣ Специальная военная операция (СВО)'}, {'datetime': {'$gte': release_date['2_1']}}], hard_link=True)
    FSB_war = await mongo_count_docs('database', 'statistics_new', [{'FSB': "Да", 'war_or_not': '2️⃣ Война'}, {'datetime': {'$gte': release_date['2_1']}}], hard_link=True)
