...
async def message_3(message: types.Message, state: FSMContext):  # Начало опроса
    ...
    try:
        text.replace('XX', war_result)
        text.replace('AA', str(round(war / FSB * 100)) if FSB >= 1 else 'N/A')
        text.replace('YY', not_war_result)
        text.replace('BB', str(round(not_war / FSB * 100)) if FSB >= 1 else 'N/A')
    except:
        text.replace('XX', 'N/A')
        text.replace('AA', 'N/A')
        text.replace('YY', 'N/A')
