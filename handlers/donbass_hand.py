...
async def donbass_chart_2(message: Message, state=FSMContext):
    ...
    await message.reply_poll("Отметьте один или более вариантов, с которыми вы согласны или частично согласны", donbass_first_poll, is_anonymous=False, allows_multiple_answers=True, reply_markup=ReplyKeyboardRemove())
...
@router.message(option_filter(option = 'ООН врет, не может быть таких жертв среди гражданского населения'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
...
@router.message(option_filter(option = 'Эти "мирные люди" — жители Украины, а значит неонацисты, которых не жалко'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
...
async def exit_point_one(message: Message, state=FSMContext):
    await redis_pop(f'Donbass_polls: First: {message.from_user.id}')
    answers = await poll_get(f'Donbass_polls: First: {message.from_user.id}')
    thing = 'Так они используют население как живой щит! Поэтому погибают мирные жители'
    if thing not in answers:
        await poll_write(f'Donbass_polls: First: {message.from_user.id}', thing)
...
@router.message(option_filter(option='Так они используют население как живой щит! Поэтому погибают мирные жители'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
...
@router.message(option_filter(option = 'Украинцам надо было просто сдаться, тогда бы стольких жертв не было'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
...
@router.message(option_filter(option = 'Это ужасно, но помимо защиты жителей Донбасса есть более весомые причины для начала войны'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
...
async def donbas_can_you_be_normal(message: Message, state=FSMContext):
    ...
    await message.reply_poll('Вернемся к текущим событиями, или у вас есть что сказать по этой теме?', donbass_second_poll, is_anonymous=False, allows_multiple_answers=True)
...
async def poll_answer_handler(poll_answer: types.PollAnswer, bot: Bot, state=FSMContext):
    ...
    for index in indexes:
        ...
        await poll_write(f'Donbass_polls: Second: {poll_answer.user.id}', donbass_second_poll[index])
    if 'Вообще-то, наших войск не было в ДНР/ ЛНР все эти 8 лет' in true_options:
        await bot.send_message(poll_answer.user.id, "Вообще-то наши войска были в ДНР и ЛНР, вот доказательства:\n\n<i>Доказательства (существуют)</i>", reply_markup=filler_kb())
        await redis_pop(f'Donbass_polls: Second: {poll_answer.user.id}')
    elif 'Путин просто помогал русскоязычному населению, которые не хотели жить в Украине после Майдана' in true_options:
        await bot.send_message(poll_answer.user.id, "Русскоязычное население Украины с удовольствием бы помогло Путину перестать быть\n\n<i>Доказательства: (существуют)</i>", reply_markup=filler_kb())
        await redis_pop(f'Donbass_polls: Second: {poll_answer.user.id}')
    elif 'Путин помог разжечь этот конфликт, чтобы помешать Украине вступить в НАТО' in true_options:
        await bot.send_message(poll_answer.user.id, "Теперь НАТО впору просить вступить в Украину. Где доказательства?..\n\n\n\n\nЗдесь: (доказателство) (доказательство)", reply_markup=filler_kb())
        await redis_pop(f'Donbass_polls: Second: {poll_answer.user.id}')
    await state.set_state(donbass_state.after_second_poll)


@router.message(second_donbass_filter(option = 'Путин просто помогал русскоязычному населению, которые не хотели жить в Украине после Майдана'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
async def donbas_no_army_here(message: Message, state=FSMContext):
    await message.answer("Русскоязычное население Украины с удовольствием бы помогло Путину перестать быть\n\n<i>Доказательства: (существуют)</i>", reply_markup=filler_kb())
    await redis_pop(f'Donbass_polls: Second: {message.from_user.id}')


@router.message(second_donbass_filter(option = 'Путин помог разжечь этот конфликт, чтобы помешать Украине вступить в НАТО'), (F.text.in_({'Договорились', "Хорошо", "Понятно"})))
async def donbas_no_army_here(message: Message, state=FSMContext):
    await message.answer( "Теперь НАТО впору просить вступить в Украину. Где доказательства?..\nЗдесь:\n\n\n\n (доказателство)\n           *кродется*\n\n(доказательство2)\n *спит*", reply_markup=filler_kb())
    await redis_pop(f'Donbass_polls: Second: {message.from_user.id}')
