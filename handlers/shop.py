...
inline1 = InlineKeyboardBuilder()
inline1.button(text='1000 x 🚀',
               callback_data='1000 x 🚀 Детская площадка', )
inline1.button(text='100 x 🏫',
               callback_data='100 x 🏫 Современная школа')
inline1.button(text='1000 x ⚡',
               callback_data='1000 x ⚡️ Электробус')
inline1.button(text='10 x 🛩',
               callback_data='10 x 🛩 Пассажирский самолёт (SuperJet)')
inline1.button(text='100 км x 🛣 ',
               callback_data='100 км x 🛣 Автомагистраль (от 4 полос)')
inline1.button(text='100 x 🌳',
               callback_data='100 x 🌳 Большой парк')
inline1.button(text='10 x 💊',
...
inline1.button(text='10 x 🏥',
               callback_data='10 x 🏥 Корпус ядерной медицины')
inline1.button(text='1 x 🔥',
               callback_data='1 x 🔥 Северный поток — 2')
inline1.adjust(3, 3, 3)
...
async def shop_welcome(message: types.Message, state: FSMContext):
    ...
    await state.update_data(why_so_many="0")
...
async def shop_bucket(message: types.Message, state: FSMContext):
    ...
    if check_text == "":
        text_bucket = f"<b>БАЛАНС</b>: <b>{change_number_format(data_dict['balance'])} руб                                                          💵</b>\n\n<b>Корзина пока пуста</b>"
    else:
        text_bucket= f"<b>БАЛАНС</b>: <b>{change_number_format(data_dict['balance'])} руб                                                          💵</b>\n\n<b>В корзине:</b>\n{check_text}"
...
async def shop_callback(query: types.CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if data[0] == "1":
        ...
        if int(balance) > 0:
            ...
            #                             reply_markup=new_inline.as_markup())
            print(check_text)
            if check_text == "":
                text_bucket = f"<b>БАЛАНС</b>: <b>{change_number_format(data_dict['balance'])} руб                                                          💵</b>\n\n<b>Корзина пока пуста</b>"
            else:
            ...
            balance = int(data_dict['balance']) + (price_dict[f'{query.data}'] * num_list[0])
            print(balance)
            ...
            print(check_text)
            if check_text == "":
                text_bucket = f"<b>БАЛАНС</b>: <b>{change_number_format(balance)} руб                                                          💵</b>\n\n<b>Корзина пока пуста</b>"
            else:
                text_bucket = f"<b>БАЛАНС</b>: <b>{change_number_format(balance)} руб                                                          💵</b>\n\n<b>В корзине:</b>\n{check_text}"
            await bot.edit_message_text(
                text=text_bucket,
